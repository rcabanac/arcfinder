...
FILE_OPTIM = open("optim.txt","w")

# we define all parameters of name_thresh
# Line 1 has parameters corresponding to the smoothing filter used on the image.
	
l1p1_smoothingsize = 7  # l1p1 - corresponds to a window size within which the smoothing 
						# is performed and this process is iterated over the complete image.
							
l1p2_mexicanb2 =  4		# l1p2 - corresponds to the b^2 in the Mexican-hat filter applied 
						# to every pixel (x,y) in the image. The filter is given by 
						# M(x,y)=exp−[(x2+y^2)/b^2] −0.5exp−[(x^2+y^2)/2b^2].
...
l2p1_elongsize = 3		# l2p1-translates to a window size in units of pixels within which 
						# the local direction of elongation is estimated. This is done by 
						# calculating second order moments of the surface brightness 
						# distribution within this window. 
						# A local elongation estimator is assigned to the pixel at the 
						# center of the window and this process is repeated for every 
						# pixel in the input image. The code uses 2×p1+1 as the window 
						# size. Typical values for p1 are 3 to 7 pixels which will 
						# translate to window sizes of 7 to 15 pixels, respectively.
						
l2p2_elongestim = 1.	# l2p2-is a fractional value for a threshold on the elongation 
						# estimator, given by p2×noise×(2p1+1). The smaller (larger) 
						# p2 values lead to fainter (brighter) detections.							
l2p3_verbose = 0		# l2p3-allows verbose printing of numbers in-between the process 
						# and ar_dt.fits 
						# which visually shows the effect of varying the parameters p1 
						# and p2. Set this parameter to 1 to turn on the verbose printing 
						# else set it to 0.
			
# Line 3 has parameters corresponding to connecting contiguous pixels with elongation 
# estimator above some thresholds and measurement of candidate arc properties. 
# The parameters up to p8 play role in the step 1 where the skeleton of the arc is 
# constructed using the elongation estimator. In step 2, the rest of the neighbouring 
# pixels, that is, the flesh of the arc is constructed for which p9 to p14 are required.
			
l3p1_windowsizeconnect = 6	# l3p1- corresponds to the window size (2p1+1) centred on a 
							# pixel within which neighbouring pixels will be connected. 
							# This process is repeated for every pixel.
							
l3p2_elongthreshold = 1.3	# l3p2-threshold on the elongation estimator to accept a pixel 
							# (within the window given by p1) as part of a candidate arc 
							# and connect with neigbouring pixels, if they satisfy the same. 
							
l3p3_minarcarea = 5 		# l3p3-minimum accepted area of the candidate arc (step 1) in 
							# units of pixels. Default value is 10 and may not require 
							# modification.
							
l3p4_maxpixcount = 1.e-10	# l3p4-maximum allowed value for the brightest pixel belonging 
							# to a candidate arc in the units of pixel counts. 
							# Since stars have extremely high pixel counts, setting a 
							# reasonable upper limit rejects any false positives in/around 
							# stars. This value should be calibrated with a test image 
							# consisting of stars and likely arc candidates.
							
l3p5_maxarcsb = 1.5			# l3p5-threshold on the mean surface brightness value per 
							# pixel and is used as p5×σ. Higher values will allow only 
							# brighter arcs to be detected. This parameter value needs to 
							# be calibrated as per the requirement.
							
l3p6_imagemargin = 3		# l3p6-number of pixels to be excluded from the boundary of 
							# the image during the execution of the code. This is useful 
							# when the images have bad/masked data at the boundaries and 
							# need not be searched for arc detection. Alternatively, one 
							# can restrict the image size to be scanned via the -w and -h 
							# option when running the code.
...
l3p8_arcminlwratio =  2.	# l3p8-minimum acceptable length-to-width ratio of the arc 
							# in units of pixels. 
...
l3p11_window = 7			# l3p11 ->  2×l3p11+1. The flesh of the arc is recovered by 
							# introducing thresholds on the mandatory input image ( g) 
							# and a duplicated image which is smoothed with a kernel ( s). 
							# This is repeated within every window (given by p11) centred 
							# on every pixel in the images. 
...
l3p14_arcmaxwidth = 10		# l3p14-, and maximum width of the final arc feature	
							# needs to satisfy. Default values (in units of pixels) may be 
							# modified.
							
l3p15_verbose = 0			# p15-default value is 0. If set to 1, will print extra numbers 
							# and ar_out.fits which allows to see the output of the code 
							# at the end of step 1 (showing the skeleton of the arc) in a 
							# FITS format. If set to 2, will print some more numbers 
							# during the process of candidate detection with some help.
# other definitions
...
# begin loop
#
for iter1 in range(4,5,1):
#	l1p2_mexicanb2 =  iter1
	for iter2 in range(5,10,5):
#		l3p2_elongthreshold = 1.3
		for iter3 in range(40,45,5):
#			l3p12_arcminarea = iter3
#
			filestatname = "challenge_stats"+str(iter1+iter2+iter3)+".txt"
			FILE_STAT = open(filestatname,"w")
...
#				image_noise = -99				# case 'n': user provided sigma - image noise - the noise is internally
				...
				name_tresh = name_thresh + "_" + str(iter1+iter2+iter3)
...
#				str(xx)+" -y "+str(yy)+" -w "+str(ww)+" -h "+str(hh)+" -o "+name_out1+"_"+str(iter1+iter2+iter3)+\
#				" -s "+name_out2+"_"+str(iter1+iter2+iter3)+" -t "+ name_tresh

				arcfinder_args = "./arcfinder.exe " +" -g "+path+"/"+filename+" -x "+\
				str(xx)+" -y "+str(yy)+" -w "+str(ww)+" -h "+str(hh)+" -o "+name_out1+"_"+str(iter1+iter2+iter3)+\
				" -s "+name_out2+"_"+str(iter1+iter2+iter3)+" -t "+ name_tresh
				...
				if l1p3_verbose > 0:
					subprocess.call("mv ar_sm.fits "+"result/ar_sm_"+input[0]+"_"+str(iter1+iter2+iter3)+".fits", shell=True)
		
				# if l2p3_verbose is on,  detection image (selection based on l2p1 and l2p2 parameters) ar_dt_*.fits are created 
	
				if l2p3_verbose > 0:
					subprocess.call("mv ar_dt.fits "+"result/ar_dt_"+input[0]+"_"+str(iter1+iter2+iter3)+".fits", shell=True)
				
				# if l3p15_verbose is on,  out image (selection based on l3p* parameters) ar_out_*.fits are created
						
				if l3p15_verbose > 0:
					subprocess.call("mv ar_out.fits "+"result/ar_out_"+input[0]+"_"+str(iter1+iter2+iter3)+".fits", shell=True)
				...
				name_plotreg = "plot_"+input[0]+"_"+str(iter1+iter2+iter3)+".reg" # ds9 region file where arc positions are stored.
				...
				FILE_OUT1 = open(name_out1+"_"+str(iter1+iter2+iter3), "r")
				...
				for i in range(0,len(line_out1)):
					...
					print name_out1+"_"+str(iter1+iter2+iter3), out1_params, fac1, fac2
...
#				subprocess.call("ds9 ar_*_"+input[0]+"_"+str(iter)+".fits "+ path+"/"+filename+" -region "+ name_plotreg+" -zoom 2 -scale zscale &", shell=True)
				...
				subprocess.call("mv "+name_plotreg+" result/", shell=True)
				#
				...
				if flagfound:
					...
					if (len(line_out1)>0):
						FILE_STAT.write(image_number+" 1 -99\n")
					else:
						FILE_STAT.write(image_number+" 0 -99\n")
					flagfound = False
			...
			FILE_LIST.close()
			FILE_STAT.close()
			FILE_STAT = open(filestatname,"r")
			array = FILE_STAT.readlines()
			notlens = 0
			lens = 0
			falsepositive = 0
			falsenegative = 0
...
#	print "id",id, "arc",arcfinder, "islens",islens
				if eval(arcfinder) == 0 and eval(islens) == 0:
					notlens = notlens + 1
				if eval(arcfinder) == 1 and eval(islens) == 1:
					lens = lens + 1
				if eval(arcfinder) == 1 and eval(islens) == 0:
					falsepositive = falsepositive + 1
				if eval(arcfinder) == 0 and eval(islens) == 1:
					falsenegative = falsenegative + 1
#			print "lens = ", lens,"\n", "not lens =",notlens,"\n","false positive = ",falsepositive, "\n", "false negative = ",falsenegative,"\n"
			FILE_OPTIM.write(name_tresh+"\n"+str(l1p2_mexicanb2)+" "+str(l3p2_elongthreshold)+" "+str(l3p12_arcminarea)+"\n")
			FILE_OPTIM.write("lens = "+str(lens)+" "+"not lens ="+str(notlens)+" "+\
			                "false positive = "+str(falsepositive)+" "+"false negative = "+\
			                str(falsenegative)+"\n")
FILE_OPTIM.close()
