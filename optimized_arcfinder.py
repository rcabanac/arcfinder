...
l3p2_elongthreshold = 1.5	# l3p2-threshold on the elongation estimator to accept a pixel 
							# (within the window given by p1) as part of a candidate arc 
							# and connect with neigbouring pixels, if they satisfy the same. 
							
l3p3_minarcarea = 10		# l3p3-minimum accepted area of the candidate arc (step 1) in 
...
filestatname = "challenge_stats.txt"
FILE_STAT = open(filestatname,"w")
FILE_STAT.write("image_number arcfinder_is_lens\n")
...
for line in FILE_LIST:
	...
	name_tresh = "thresh.data"		# case 't': threshold setting filename
...
#				" -s "+name_out2+"_"+str(iter1+iter2+iter3)+" -t "+ name_tresh
	...
	arcfinder_args = "./arcfinder.exe " +" -g "+path+"/"+filename+" -x "+\
				str(xx)+" -y "+str(yy)+" -w "+str(ww)+" -h "+str(hh)+" -o "+name_out1+\
				" -s "+name_out2+" -t "+ name_tresh
	...
	if l1p3_verbose > 0:
		subprocess.call("mv ar_sm.fits "+"result/ar_sm_"+input[0]+".fits", shell=True)
		
	# if l2p3_verbose is on,  detection image (selection based on l2p1 and l2p2 parameters) ar_dt_*.fits are created 
	
	if l2p3_verbose > 0:
		subprocess.call("mv ar_dt.fits "+"result/ar_dt_"+input[0]+".fits", shell=True)
				
	# if l3p15_verbose is on,  out image (selection based on l3p* parameters) ar_out_*.fits are created
						
	if l3p15_verbose > 0:
		subprocess.call("mv ar_out.fits "+"result/ar_out_"+input[0]+".fits", shell=True)
	...
	name_plotreg = "plot_"+input[0]+".reg" # ds9 region file where arc positions are stored.
	...
	for i in range(0,len(line_out1)):
		...
		print name_out1, out1_params, fac1, fac2
...
#				subprocess.call("ds9 ar_*_"+input[0]+"_"+str(iter)+".fits "+ path+"/"+filename+" -region "+ name_plotreg+" -zoom 2 -scale zscale &", shell=True)
	...
	if (len(line_out1)>0):
		FILE_STAT.write(image_number+" 1\n")
	else:
		FILE_STAT.write(image_number+" 0\n")		
	#
	# end loop 
	#
FILE_LIST.close()
